...
def _locate_image_file( self, tex, file_name, packageName, default_extensions, query_extensions=True ):
	f = file_name

	ext = list(default_extensions)
	if query_extensions:
		ext = self.ownerDocument.userdata.getPath(
			'packages/%s/extensions' % packageName,
			ext )

	paths = self.ownerDocument.userdata.getPath(
					'packages/%s/paths' % packageName, ['.'])
	img = None
