...
\lineii{str}{expands all macros then sets the value of the argument
   in the \member{attributes} dictionary to the string content of the
   argument}. If the subtype is source, then the string must be a
	simple string containing no macros, which will be turned into a
    simple string; this allows special characters like $ and \% to be
    escaped.
